...
![Общая схема сборки](http://img-fotki.yandex.ru/get/6814/158800653.0/0_1067da_f33ee15d_orig)
...
Сразу после применения команды начинается первый этап сборки: на основе стандартной JavaScript-декларации бандла создаётся файл декларации в специфическом БЭМ-формате - <i>*.bemdecl.js</i>. За счёт устранения повторяющихся отрезков кода уменьшается итоговый объём списка, который в оптимальной форме перечисляет используемые блоки, элементы и модификаторы. При этом вложенность сущностей не учитывается, но порядок их появления сохранён.
